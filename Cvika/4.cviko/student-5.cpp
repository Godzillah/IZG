...
void initControlPointsDown(S_Vector **points, int offset_x, int offset_y) {
  ...
  *points = vecCreateEmpty(sizeof(Point2d));
  ...
  p.x = 331; p.y = 180; point2d_vecPushBack(*points, p);
  
  p.x = 360; p.y = 0;    point2d_vecPushBack(*points, p);
 
  p.x = 379; p.y = -120;  point2d_vecPushBack(*points, p);
  p.x = 441; p.y = -120;  point2d_vecPushBack(*points, p);
  
  p.x = 460; p.y = 0;    point2d_vecPushBack(*points, p);

  p.x = 471; p.y = 70;  point2d_vecPushBack(*points, p);
  p.x = 524; p.y = 70;  point2d_vecPushBack(*points, p);
  
  p.x = 535; p.y = 0;    point2d_vecPushBack(*points, p);

  p.x = 541; p.y = -40;  point2d_vecPushBack(*points, p);
  p.x = 579; p.y = -40;  point2d_vecPushBack(*points, p);
...
void bezierCubic(const Point2d *P0, const Point2d *P1, const Point2d *P2, const Point2d *P3, 
   ...
   */
  for (double t = 0; t < 1; t+=1.0/quality) { 
    double B0, B1, B2, B3;
    B0 = pow(1 - t, 3);
    B1 = 3 * t * pow(1 - t, 2);
    B2 = 3 * pow(t, 2) * (1 - t);
