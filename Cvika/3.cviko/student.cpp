...
void pinedaTriangle(const Point &v1, const Point &v2, const Point &v3, const RGBA &color1, const RGBA &color2, bool arrow)
{
    ...
    //////// DOPLNTE KOD /////////
	//pripad, ze x je zaporne, urobime z neho 0
	minX = MAX(0, minX);
	minY = MAX(0, minY);
	...
	edgeParams[0].deltaX = v2.x - v1.x;
	edgeParams[0].deltaY = v2.y - v1.y;
	edgeParams[1].deltaX = v3.x - v2.x;
	edgeParams[1].deltaY = v3.y - v2.y;
	edgeParams[2].deltaX = v1.x - v3.x;
	edgeParams[2].deltaY = v1.y - v3.y;*/
	...
	int edgeF1 = deltaX1*(minY - v1.y) - deltaY1*(minX - v1.x);
	int edgeF2 = deltaX2*(minY - v2.y) - deltaY2*(minX - v2.x);
	int edgeF3 = deltaX3*(minY - v3.y) - deltaY3*(minX - v3.x);
	...
	for (int y = minY; y <= maxY; y++) {
		bool even = (y - minY) % 2 == 0; //dostanem 0 ked som na sudem radku
...
void pinedaPolygon(const Point *points, const int size, const RGBA &color1, const RGBA &color2)
{
	...
	for (int i = 0; i < size; i++) {
		edgeFncVal[i] = edgeParams[i].deltaX * (minY - points[i].y) - edgeParams[i].deltaY * (minX - points[i].x);
	}
	...
	for (int y = minY; y <= maxY; y++) {
		...
		for (int x = (even) ? minX : maxX; (even) ? x <= maxX : x >= minX; x += (even) ? 1 : -1) {
			for (int i = 0; i < size; i++) {
				if (edgeFncVal[i] < 0) edgeFncOK = false;
			}
			if (edgeFncOK) putPixel(x, y, color1);
			edgeFncOK = true;
			if ((even && x == maxX) || (!even && x == minX)); {
				for (int i = 0; i < size; i++) {
