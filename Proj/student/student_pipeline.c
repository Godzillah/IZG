...
#define X1 float x1 = vertices[0].data[0]
#define X2 float x2 = vertices[1].data[0]
#define X3 float x3 = vertices[2].data[0]
#define Y1 float y1 = vertices[0].data[1]
#define Y2 float y2 = vertices[1].data[1]
#define Y3 float y3 = vertices[2].data[1]
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  if(head->enabled == 1){
    return (uint8_t*)head->buffer + gl_VertexID * head->stride + head->offset;
  }
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  VertexIndex indexVer = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);
  int i = 0;
  while(i < MAX_ATTRIBUTES){
    if (puller->heads[i].enabled != 0){
      output->attributes[i] = gpu_computeVertexAttributeDataPointer(&(puller->heads[i]), indexVer);
    }
    else{
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float centerX = pixelCenter->data[0];
  float centerY = pixelCenter->data[1];

  X1;X2;X3;Y1;Y2;Y3; // macros for defining all x , y

  coords->data[0] = ((y2 - y3) * (centerX - x3) + (x3 - x2) * (centerY - y3)) / ((y2 - y3) * (x1 - x3) + (x3 - x2) * (y1 - y3));
  coords->data[1] = ((y3 - y1) * (centerX - x3) + (x1 - x3) * (centerY - y3)) / ((y2 - y3) * (x1 - x3) + (x3 - x2) * (y1 - y3));
