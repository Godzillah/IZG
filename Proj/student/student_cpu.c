...
///  - cpu_setIndexing()
	...
	// Uniform vars....
  cpu_reserveUniform(phong.gpu, "projectionMatrix", UNIFORM_MAT4);    //Gpu <--> Uniform name <--> Uniform type
  cpu_reserveUniform(phong.gpu, "viewMatrix", UNIFORM_MAT4);          //Gpu <--> Uniform name <--> Uniform type
  cpu_reserveUniform(phong.gpu, "cameraPosition", UNIFORM_VEC3);      //Gpu <--> Uniform name <--> Uniform type
  cpu_reserveUniform(phong.gpu, "lightPosition", UNIFORM_VEC3);       //Gpu <--> Uniform name <--> Uniform type


  phong.programId = cpu_createProgram(phong.gpu);


	// attach vertex and fragment shaders...
  cpu_attachVertexShader(	phong.gpu, phong.programId, phong_vertexShader);
  cpu_attachFragmentShader( phong.gpu, phong.programId, phong_fragmentShader);

	// setting atribute interpolations...
  cpu_setAttributeInterpolation( phong.gpu, phong.programId, 0, ATTRIB_VEC3, SMOOTH);
  cpu_setAttributeInterpolation( phong.gpu, phong.programId, 1, ATTRIB_VEC3, SMOOTH);

	// creating buffers/loading data...
  cpu_createBuffers(phong.gpu,1, &phong.bufferId);

  cpu_bufferData(phong.gpu,phong.bufferId,sizeof(bunnyVertices),(void *)bunnyVertices);

	// creating vertex pullers...
  cpu_createVertexPullers(phong.gpu,1,&phong.pullerId);

	// set vertex puller heads/enable
  cpu_setVertexPullerHead(phong.gpu, phong.pullerId, 0, phong.bufferId, 0, sizeof(float) * 6);
  cpu_setVertexPullerHead(phong.gpu, phong.pullerId, 1, phong.bufferId , sizeof(float) * 3, sizeof(float) * 6);

  cpu_enableVertexPullerHead(phong.gpu,phong.pullerId,0);
  cpu_enableVertexPullerHead(phong.gpu,phong.pullerId,1);

  cpu_createBuffers(phong.gpu,1,&phong.bufferIdx);

  cpu_bufferData(phong.gpu, phong.bufferIdx, sizeof(bunnyIndices), (void *)bunnyIndices);

  cpu_setIndexing(phong.gpu, phong.pullerId, phong.bufferIdx, 2);
}
...
///  - getUniformLocation()
  ...
  cpu_useProgram(phong.gpu, phong.programId);

  cpu_uniformMatrix4fv(phong.gpu, getUniformLocation(phong.gpu, "projectionMatrix"), (const float*)&projectionMatrix);

  cpu_uniformMatrix4fv(phong.gpu, getUniformLocation(phong.gpu, "viewMatrix"), (const float*)&viewMatrix);

  cpu_bindVertexPuller(phong.gpu, phong.pullerId);

  cpu_drawTriangles(phong.gpu, (sizeof(bunnyIndices) / sizeof(VertexIndex)));

  cpu_uniform3f(phong.gpu, getUniformLocation(phong.gpu, "lightPosition"), phong.lightPosition.data[0], phong.lightPosition.data[2], phong.lightPosition.data[2]);
